...
const MOTION_CONFIG = {
  ...
  animate: { opacity: 1, y: 0 },
  exit: { opacity: 0, y: 20 },
  transition: { duration: 0.3, ease: "easeOut" as const }, // ✅ Fix here
};
...
export default function Chat({ isFloating = true, initialMessage }: ChatProps) {
      ...
      setMessages([
        {
          ...
          parts: [
            {
              type: "text",
              text: "Hello! I'm Sagar's AI assistant. How can I help you today?",
            },
          ],
        },
      ]);
  ...
  useEffect(() => {
    ...
    if (isFloating && aiCommand && lastText !== aiCommand) {
      sendMessage({
        role: "user",
        id: nanoid(),
        parts: [{ type: "text", text: aiCommand }],
      });
      setAICommand("");
    }
  }, [aiCommand, isFloating, sendMessage, messages, setAICommand]);
